...
public class BinaryLogConfigParser extends AbstractLogConfigParser {
    ...
    @Override
    protected LogConfig doParse(String outputPath, boolean autoFlush, int maxBufferSize, Map<String, Object> logConf, Map<String, Object> defaults) {
        if (STDOUT.equals(outputPath))
            throw new IllegalArgumentException("Output path can not be stdout.");
        return new BinaryLogConfig(outputPath, autoFlush, maxBufferSize);
    }
}
